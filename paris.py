...
def paris_cross(f):
    ...
    elapsed_time = 0
    score = 0
    while elapsed_time < T:  # While there is still time
        path.append(node)
        new_neighbors = list(filter(lambda neighbor: not visited[node][neighbor], paris[node]))
        if new_neighbors:
            ...
            dist, prec = dijkstra(paris, weight, node)
            for jump_time, node in sorted((dist[node], node) for node in range(N)):
                if any(not visited[node][neighbor] for neighbor in paris[node]):
                    chosen_neighbor = node
                    elapsed_time += jump_time
                    break
            # print('should jump at', chosen_neighbor, 'in', jump_time, 'seconds')
        node = chosen_neighbor
    print('Score:', score, 'in', elapsed_time, 'seconds')
    return path

# paris_cross(node_value)
